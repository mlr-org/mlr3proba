...
prediction$distr[1]$survival(90:105)
...
# installing a learner from the mlr3learners repo
...
# rpart does return a distr prediction so we will overwrite it with our own composition
leaner_lp = lrn("surv.rpart")
...
pod = po("distrcompose", param_vals = list(form = "ph", overwrite = TRUE))
...
rpart.distr = distrcompositor(learner = lrn("surv.rpart"),
                             estimator = "kaplan",
                             form = "ph",
                             overwrite = TRUE)
rpart.distr$train(task)$predict(task)
```
...
composed_lrn_rpart = distrcompositor(lrn("surv.rpart"), "kaplan", "ph")

lrns = lapply(paste0("surv.", c("kaplan", "coxph")), lrn)

design = benchmark_grid(tasks = task, learners = c(lrns, list(composed_lrn_rpart)),
                        resamplings = rsmp("cv", folds = 2))

bm = benchmark(design)
bm$aggregate(lapply(c("surv.cindex","surv.graf","surv.grafSE"), msr))[,c(4, 7:9)]
```
