...
#' <% times_str = "* `times` :: `vector()` \\cr If `integrate == TRUE` then a vector of time-points over which to integrate the score, or a single time point at which to return the score." %>
...
#'
#' @section Fields:
#' <% inherit = get(fullname)$inherit %>
#'
#' See [MeasureSurv] <%= if(inherit == "MeasureSurvAUC") "and \\link{MeasureSurvIntegrated}" %>
#' <%= if(inherit != "MeasureSurv" & inherit != "MeasureSurvAUC") paste0("and \\link{", inherit, "}")%>
#'
