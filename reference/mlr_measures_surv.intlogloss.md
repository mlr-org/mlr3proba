...
Calculates the **Integrated Survival Log-Likelihood** (ISLL) or
Integrated Logarithmic (log) Loss, aka integrated cross entropy.
...
\$\$L\_{ISLL}(S_i, t_i, \delta_i) = - \int^{\tau^\*}\_0
\frac{log\[1-S_i(\tau)\] \text{I}(t_i \leq \tau, \delta_i=1)}{G(t_i)} +
\frac{\log\[S_i(\tau)\] \text{I}(t_i \> \tau)}{G(\tau)} \\ d\tau\$\$
...
ISLL is not a proper scoring rule, see Sonabend et al. (2024) for more
...
- [`MeasureSurvIntLogloss$new()`](#method-MeasureSurvIntLogloss-new)

- [`MeasureSurvIntLogloss$clone()`](#method-MeasureSurvIntLogloss-clone)
