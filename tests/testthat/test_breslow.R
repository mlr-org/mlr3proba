...
test_that("surv_breslow works", {
  ...
  # normal test
  surv = surv_breslow(times = t$times(p$train), status = t$status(p$train),
  ...
  surv2 = surv_breslow(times = t$times(p$train), status = t$status(p$train),
  ...
  surv3 = surv_breslow(times = t$times(p$train), status = t$status(p$train),
  ...
  expect_error(surv_breslow(times = NULL))
  expect_error(surv_breslow(times = c(1,2), status = 0, lp_train = 1))
  expect_error(surv_breslow(times = c(1,2), status = c(0,1), lp_train = 1))
  expect_error(surv_breslow(times = c(1,2), status = c(0,1), lp_train = c(1,2)))
  expect_error(surv_breslow(times = c(1,2), status = c(0,1), lp_train = c(1,2)))
})
