...
test_that("surv_return works correctly when input is survival matrix or array", {
  ...
  surv_mat_median = apply(surv_array, c(1,2), median)
...
test_that("edge cases work", {
  ...
  min_mat1 = matrix(surv_array[1:3,1,1], nrow = 3, dimnames = list(NULL, 1))
  ...
  min_mat2 = matrix(min_vec2, nrow = 1, dimnames = list(NULL, names(min_vec2)))
  ...
  arrmin1 = surv_array[1:3,1,, drop = FALSE]
  ...
  arrmin2 = surv_array[1,1:3,, drop = FALSE]
  ...
  arrmin3 = surv_array[1,1,1, drop = FALSE]
  res_arr3 = .surv_return(surv = arrmin3, which.curve = 'mean')
  expect_equal(res_arr3$distr, arrmin3)
  expect_equal(res_arr3$crank, unname(-log(arrmin3[,,1]))) # expected mortality
})
